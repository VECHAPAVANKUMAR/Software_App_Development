...
def search() :
    ...
    if request.method == "POST" :
        ...
        if session.get("user_email") :
            ...
            try :
                books[0].isbn
                return render_template("search1.html", books=books)
            except Exception as exc :
                flash("No Results Found")
                return render_template("search1.html", books=[])
        else :
            flash("Please Login", "info")
            return redirect(url_for("login"))
...
def bookpage(isbn) :
    ...
    if request.method == "GET" :
        if session.get("user_email") :
            ...
            # reviews = Review.session.query.filter(Review.isbn=isbn).first().reviews
            # return render_template("reviews.html", isbn=isbn, book=book, reviews=reviews)
